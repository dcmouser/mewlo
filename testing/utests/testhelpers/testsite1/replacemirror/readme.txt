In the configuration of the site, we typically instruct mewlo that this directoriy is a potential mirror for mewlo assets.
...
NOTE: Mewlo has an ability to copy files from mpack directories into an external static asset serving directory (where another web server will see and serve the files); this is done AFTER the replacement-mirroring specified in this directory, and so the files exposed and copied will be the replaced files (as one would want).

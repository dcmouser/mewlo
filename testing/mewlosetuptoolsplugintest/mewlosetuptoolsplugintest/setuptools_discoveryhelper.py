...
Note that they depend on using the __file__ macro, so this discoveryhelper.py module MUST be in the location where the mpack source code is.

IMPORTANT: We don't need this file if we choose to use the simple way of sepecify entry_points using 'moduleforpath'.
In the test code I use, these functions are *not* called or used; they just exist as examples of alternate ways of telling the setuptools plugin which files are mpack files.
...
def get_infofiles():
    ...
    return [ thismoduledir+'/mewlotestplug_mpack.json' ]

...
class MewloPackManager(manager.MewloManager):
    ...
    def __init__(self, mewlosite, debugmode):
        ...
        self.set_filepatternsuffix(self.DefMewlo_Pack_filepatternsuffix)
    ...
    def createadd_pack_frominfofile(self, infofilepath, eventlist):
        ...
        if (pkg != None):
            ...
            pkg.load_infofile()
            # now add it to hash
            apackid = pkg.get_ourinfofile_property(MewloPack.DEF_INFOFIELD_uniqueid)
    ...
    def startup_pack_auto(self, mewlosite, pack, eventlist):
        ...
        packid = pack.get_ourinfofile_property(MewloPack.DEF_INFOFIELD_uniqueid)
        ...
        elif (pack.get_ourinfofile_property(MewloPack.DEF_INFOFIELD_isrequired)):
    ...
    def want_enable_pack(self, pack, eventlist):
        ...
        flag_enable = get_value_from_dict(packsettings,MewloSettings.DEF_SETTINGNAME_isenabled)
        # is the pack REQUIRED?
        if (pack.get_ourinfofile_property(MewloPack.DEF_INFOFIELD_isrequired) and flag_enable != False):
            # set to enable if enabled explicitly or nothing specified
            reason = "required pack"
            flag_enable = True
        elif (flag_enable==None):
            flag_enable = get_value_from_dict(self.default_packsettings, MewloSettings.DEF_SETTINGNAME_isenabled)
    ...
    def check_pack_dependencies(self, pack, requirer, assumegoodlist):
        ...
        requiredict = pack.get_ourinfofile_property(MewloPack.DEF_INFOFIELD_requires)
        if (requiredict != None):
            # ok let's check for required PACKAGES (not python packs but our packs)
            requiredpacks = get_value_from_dict(requiredict, MewloPack.DEF_INFOFIELD_requiredpacks)
